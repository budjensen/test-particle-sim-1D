...
def test_isothermal_relaxation():
    ...
    ions = Species(q=constants.q_e, m=constants.m_p, name="proton", capacity=n_particles)
    ions.initialize_maxwellian(
        n=n_particles,
        z_min=0.0, z_max=0.1,
        temperature={"eV": T_ion_init_eV},
        seed=42
    )
...
def test_velocity_isotropization():
    ...
    print(f"[Isotropization] End   Tx: {Tx_final:.0f}, Ty: {Ty_final:.0f}, Tz: {Tz_final:.0f} K")
...
def test_distribution_relaxation():
    ...
    ions.initialize_uniform(n_particles, z_min=0.0, z_max=0.1, v0=[0.0, 0.0, 0.0], seed=42)
    ...
    print(f"[Distribution] End Avg Profile T: {avg_profile_T:.0f} K (Target: {T_neutral_K:.0f})")
    ...
    kT = T_neutral_eV
    theory_pdf = (2 * np.sqrt(centers / np.pi) * (1 / kT)**(1.5) * np.exp(-centers / kT))
...
def test_density_profile_reconstruction():
    ...
    assert abs(avg_density_right - expected_density_right) < 0.05 * expected_density_right
