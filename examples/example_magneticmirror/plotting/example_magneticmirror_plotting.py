"""
example_magneticmirror_plotting.py

Combined loader + plotting for the magnetic mirror example.

It:
1. Loads example_magneticmirror_results.npz from the ./results directory.
2. Generates:
   - Temperature history
   - Energy distribution
...
def main() -> None:
    ...
    # examples/
    #   example_mageticmirror/
    #       example_magneticmirror_main.py
    #       results/
    #           example_magneticmirror_results.npz
    #       plotting/
    #           example_magneticmirror_plotting.py  <-- this file
    base_dir = Path(__file__).resolve().parent  # .../example_mageticmirror/plotting
    example_dir = base_dir.parent  # .../example_mageticmirror
    results_dir = example_dir / "results"  # .../example_mageticmirror/results
    plots_dir = base_dir

    results_path = results_dir / "example_magneticmirror_results.npz"

    if not results_path.is_file():
        msg = (
            f"Could not find results file at: {results_path}\n"
            "Make sure you've run example_magneticmirror_main.py first."
        )
