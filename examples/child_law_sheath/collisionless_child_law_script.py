"""
child_law_sheath/collisionless_child_law_script.py

Demonstration script: 300K ions traversing a collisionless sheath
...
def store_and_remove_particles(
    ...
    """
    # Create buffer
    remove_mask = species.z[: species.N] >= z_max
    buffer = np.column_stack(
        (
            species.vx[: species.N][remove_mask],
            species.vy[: species.N][remove_mask],
            species.vz[: species.N][remove_mask],
        )
    )
...
def run_child_sheath_example() -> None:
    ...
    # Remove any zero entries at the end of the diagnostics array
    traj_hist = traj_hist[:sample_idx, :]
    time_hist = time_hist[:sample_idx]

    results_dir = Path(__file__).resolve().parent / "collisionless_results"
