...
1. Global quantities (scalar temperature).
...
def compute_global_temperature(species: particles.Species) -> float:
    ...
    T = (species.m * total_variance) / (3 * constants.kb)

    return T


def compute_component_temperatures(species: particles.Species) -> Tuple[float, float, float]:
    """
